...
__global__ void compute_convolution(double* O, double* I, double* F) {
    ...
    __shared__ double tile_F[block_z][C][FW][FH];
    int tile_y = threadIdx.x;
    int tile_x = threadIdx.y;
    int tile_k = threadIdx.z;
    int y = threadIdx.x + blockIdx.x * blockDim.x;
    int x = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.z + blockIdx.z * blockDim.z;
    for (int c = 0; c < C; c++) {
        if (tile_x < FW && tile_y < FH) {
            tile_F[tile_k][c][tile_x][tile_y] = F[idx_F(k, c, tile_x, tile_y)];
        }
...
//    printf("%d %d %d\n", x, y, k);
    //for (int k = 0; k < K; k++) {
        ...
        for (int c = 0; c < C; c++) {
            for (int i = 0; i < FH; i++)
                for (int j = 0; j < FW; j++)
                    value += tile_F[tile_k][c][FW - 1 - i][FH - 1 - j] * tile_I[c][tile_x+i][tile_y+j];
        }
