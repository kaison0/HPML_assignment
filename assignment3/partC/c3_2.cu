...
//    if (argc == 4) {
 ...
 //       block_y = std::stoi(argv[2]);
  ...
  //  }
    ...
    long I_bytesize = (long)C*H*W* sizeof(double);
    ...
    cudnnFindConvolutionForwardAlgorithm(handle, input_desc, filter_desc, conv_desc, output_desc, 10, &returned_algo_count, perf_results);
    ...
    size_t ws_size = 0;
    cudnnGetConvolutionForwardWorkspaceSize(handle, input_desc, filter_desc, conv_desc, output_desc, algo, &ws_size);
